...
const validateToken = async token => {
	try {
		await github.auth({type: 'token', token});

		// See if the token works by getting the data for our company's account
		await github.users.getByUsername({username: 'vercel'});
	} catch (_) {
		return false;
	}
	return true;
