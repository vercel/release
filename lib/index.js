...
const {load: loadPlugins} = require('../lib/plugins')
const steps = require('../lib/steps')

args
  .option('pre', 'Mark the release as prerelease')
  .option('overwrite', 'If the release already exists, replace it')
  .option(['m', 'plugins'], 'List of plugins separated by ":"')

const flags = args.parse(process.argv)
const plugins = loadPlugins(args.plugins)
...
const checkReleaseStatus = async release => {
  ...
  githubConnection.repos.getReleases({
    ...
    for (const rel of response) {
      if (rel.tag_name === release.tags.latest.tag) {
        existingRelease = rel
        break
      }
    }
    ...
    if (flags.overwrite) {
      ...
      collectChanges(release.tags, existingRelease.id)
...
const main = async () => {
  ...
  await plugins.run(steps.inferRange, release)
  // TODO: more steps
